...
if busted_poise?
  describe package(python_package_prefix(node) + '-pip') do
    it { should be_installed }
  end
  describe package(python_package_prefix(node) + '-setuptools') do
    it { should be_installed }
  end
  describe package(python_package_prefix(node) + '-wheel') do
    it { should be_installed }
  end
else
